...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String BASE_URL = "https://63fa-73-109-142-190.ngrok-free.app/";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        patchButton.setOnClickListener(v -> {
            ...
            updatedApiResponse.setFirstName(firstNameInput.getText().toString());
            updatedApiResponse.setLastName(lastNameInput.getText().toString());
            updatedApiResponse.setAddress(addressInput.getText().toString());
            updatedApiResponse.setRollNumber(Integer.parseInt(rollNumberInput.getText().toString()));
            updatedApiResponse.setMobile(mobileInput.getText().toString());

            // Send a PATCH request to update the entry
            Call<ApiResponse> call = apiService.patchApiResponse(id, updatedApiResponse);
            call.enqueue(new Callback<ApiResponse>() {
                @Override
                public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        // Display the response from the server
                        textView.setText("Patch Response was successful");
                    } else {
                        // Indicate that the PATCH request was not successful
                        textView.setText("Patch Response was not successful");
                    }
                }
